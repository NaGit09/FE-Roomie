    ...
    <div className="space-y-10 text-slate-800 font-sans w-full">
      ...
      {loading ? (
        ...
        <div className="rounded-[2.5rem] border border-slate-200 bg-slate-50 p-16 text-center space-y-4 max-w-md mx-auto">
          ...
          <h3 className="text-sm font-bold text-slate-700">Không tìm thấy giao dịch nào</h3>
            ...
            <tbody className="divide-y divide-white/2 text-slate-600">
              {filteredOrders.map((order) => (
                <tr key={order.id} className="hover:bg-white/2 transition-colors">
                  ...
                  <td className="py-4 px-4 font-mono font-semibold text-slate-650 text-[10px]" title={order.user_id}>
                  ...
                  <td className="py-4 px-6 pr-8 text-right">
                    <button
                      ...
                      className="h-8 px-3 rounded-lg bg-slate-100 border border-slate-200 hover:bg-pink-500/20 hover:text-pink-400 hover:border-pink-500/30 text-slate-650 flex items-center justify-center cursor-pointer transition-all active:scale-90 text-[10px] font-bold uppercase tracking-wider gap-1.5 ml-auto"
                    >
                ...
                <div className="space-y-4">
                  <div className="rounded-2xl border border-slate-200 bg-card/60 p-4 space-y-3">
                    <span className="text-slate-650 font-bold uppercase tracking-wider block text-[9px] flex items-center gap-1.5">
                      <CreditCard className="h-3.5 w-3.5 text-slate-650" />
                      Thông tin hóa đơn
                    </span>
                    <div className="space-y-2 text-slate-600 font-medium">
                      <div>
                        <span className="text-[8px] text-slate-650 block uppercase">Mã khách hàng (User ID)</span>
                        <span className="font-mono font-bold text-slate-700">{selectedOrder.user_id}</span>
                      </div>
                      <div>
                        <span className="text-[8px] text-slate-650 block uppercase">Gói cước liên kết</span>
                        <span className="font-extrabold text-slate-700">{getItemName(selectedOrder.item_type, selectedOrder.item_id)}</span>
                      </div>
                      <div>
                        <span className="text-[8px] text-slate-650 block uppercase">Tổng tiền thanh toán</span>
                        <span className="font-black text-[#FBBF24] text-sm">{formatVND(selectedOrder.total_amount)}</span>
                      </div>
                    </div>
                  </div>

                  <div className="rounded-2xl border border-slate-200 bg-card/60 p-4 space-y-2 text-slate-350">
                  ...
                  <form onSubmit={handleUpdateStatus} className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        className="w-full h-11 border border-slate-200 bg-background rounded-xl px-4 text-slate-700 outline-none focus:border-pink-500/50 cursor-pointer text-xs font-bold"
                      >
