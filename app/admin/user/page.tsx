    ...
    <div className="space-y-10 text-slate-800 font-sans w-full">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
        {[
          ...
          <div
            key={idx}
            className="rounded-2xl border border-slate-200 bg-card/60 backdrop-blur-md p-4 text-left space-y-1.5 shadow-md"
          >
      ...
      {/* 3. Filter Controls */}
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-4 rounded-3xl bg-card/40 border border-slate-200">
        {/* Search */}
        <div className="flex items-center gap-2 h-11 bg-card/60 rounded-xl px-3.5 border border-slate-200 text-xs text-slate-650 w-full md:max-w-xs focus-within:border-primary/50 transition-colors">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="h-9 border border-slate-200 bg-background rounded-lg px-3 text-slate-700 outline-none text-[10px] font-bold cursor-pointer"
            >
      ...
      {loading ? (
        ...
        <div className="rounded-[2.5rem] border border-slate-200 bg-slate-50 p-16 text-center space-y-4 max-w-md mx-auto">
          ...
          <h3 className="text-sm font-bold text-slate-700">Không tìm thấy thành viên</h3>
            ...
            <tbody className="divide-y divide-white/2 text-slate-600">
              {filteredUsers.map((user) => (
                <tr key={user.id} className="hover:bg-white/2 transition-colors">
                  <td className="py-4 px-6 pl-8 font-mono font-bold text-slate-650 text-[10px]">
                    {user.id}
                  </td>
                  ...
                  <td className="py-4 px-4 font-mono font-bold text-slate-600 text-center md:text-left pl-6">
                  ...
                  <td className="py-4 px-6 pr-8 text-right">
                    <button
                      ...
                      className="h-8 px-3 rounded-lg bg-slate-100 border border-slate-200 hover:bg-blue-500/20 hover:text-blue-400 hover:border-blue-500/30 text-slate-650 flex items-center justify-center cursor-pointer transition-all active:scale-90 text-[10px] font-bold uppercase tracking-wider gap-1.5 ml-auto"
                    >
                ...
                <div className="space-y-4">
                  <div className="rounded-2xl border border-slate-200 bg-card/60 p-4 space-y-3">
                    ...
                    <div className="space-y-2 text-slate-600 font-medium">
                      <div className="flex items-center gap-2">
                        <Mail className="h-4 w-4 text-slate-650 shrink-0" />
                        <span>{selectedUser.email}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Calendar className="h-4 w-4 text-slate-650 shrink-0" />
                        <span>Đăng ký: {new Date(selectedUser.created_at).toLocaleDateString("vi-VN")}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Activity className="h-4 w-4 text-slate-650 shrink-0" />
                  ...
                  {selectedUser.role === "LANDLORD" && selectedUser.landlord_profile && (
                    <div className="rounded-2xl border border-slate-200 bg-card/60 p-4 space-y-3">
                      <span className="text-primary font-bold uppercase tracking-wider block text-[9px] flex items-center gap-1.5">
                        <ShieldCheck className="h-3.5 w-3.5" />
                        Hồ sơ chủ nhà (Landlord Profile)
                      </span>
                      <div className="space-y-1.5 text-slate-600">
                        <div>
                          <span className="text-[9px] text-slate-650 block uppercase">Số điện thoại liên hệ</span>
                          <span className="font-bold text-xs">{selectedUser.landlord_profile.phonenumber || "N/A"}</span>
                        </div>
                        <div>
                          <span className="text-[9px] text-slate-650 block uppercase">Facebook Link</span>
                          {selectedUser.landlord_profile.facebook ? (
                            ...
                            <span className="font-semibold text-slate-650">N/A</span>
                          )}
                        </div>
                      </div>
                    </div>
