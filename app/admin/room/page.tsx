    ...
    <div className="space-y-10 text-slate-800 font-sans">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {[
          ...
          { label: "Còn trống", value: vacantRoomsCount, sub: "Sẵn sàng đón renter", color: "text-amber-500", bg: "bg-primary/10" },
          ...
          <div key={idx} className="rounded-2xl border border-slate-200 bg-card/60 backdrop-blur-md p-4 text-left space-y-1.5 shadow-md relative">
            {m.pulse && (
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-[9px] text-slate-650 font-bold uppercase tracking-wider font-body">{m.sub}</span>
            </div>
          </div>
        ))}
      </div>
            ...
            >
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {getRoomFeatures(room).slice(0, 3).map((feat: string, fIdx: number) => (
                    <span
                      key={fIdx}
                      className="inline-flex text-[9px] font-semibold text-slate-650 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded"
                    >
                      {feat}
                    </span>
                  ))}
                  {getRoomFeatures(room).length > 3 && (
                    <span className="inline-flex text-[8px] font-black text-slate-650 px-2 py-0.5 rounded font-body">
              ...
              <div className="mt-6 pt-4 border-t border-dashed border-slate-200 flex items-center justify-between">
                <div className="space-y-0.5 text-left">
                  <span className="text-[8px] font-black text-slate-650 uppercase tracking-widest block font-body">Biểu phí tháng</span>
                  <span className="text-sm font-black text-[#FBBF24] font-sans">
                    {formatVND(room.price)}
                    <span className="text-[9px] text-slate-650 font-bold uppercase tracking-wider"> / th</span>
                  </span>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
            ...
            >
              ...
              <div className="space-y-1.5 text-left border-b border-slate-200 pb-4">
                <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[8px] font-black uppercase tracking-widest border ${
                  selectedRoom.status === "PENDING" ? "bg-red-500/10 border-red-500/30 text-red-400" : "bg-primary/10 border-primary/30 text-primary"
                }`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs text-left">
                <div className="space-y-4">
                  <div>
                    <span className="text-slate-650 font-bold uppercase tracking-wider block text-[9px]">Chủ sở hữu (Owner ID)</span>
                    <span className="font-mono font-bold text-slate-350 text-slate-600 break-all">{selectedRoom.owner_id || "Quản trị viên"}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <span className="text-slate-650 font-bold uppercase tracking-wider block text-[9px]">Giá thuê phòng</span>
                      <span className="font-extrabold text-[#FBBF24] text-sm">{formatVND(selectedRoom.price)}</span>
                    </div>
                    <div>
                      <span className="text-slate-650 font-bold uppercase tracking-wider block text-[9px]">Đặt cọc</span>
                      <span className="font-extrabold text-slate-700 text-sm">{formatVND(selectedRoom.deposit || selectedRoom.price)}</span>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <span className="text-slate-650 font-bold uppercase tracking-wider block text-[9px]">Diện tích</span>
                      <span className="font-extrabold text-slate-700">{selectedRoom.area} m²</span>
                    </div>
                    <div>
                      <span className="text-slate-650 font-bold uppercase tracking-wider block text-[9px]">Sức chứa thực tế</span>
                      <span className="font-extrabold text-slate-700">{getRoomOccupied(selectedRoom)} / {getRoomCapacity(selectedRoom)} Renter</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="pt-4 border-t border-slate-200 flex justify-end gap-3">
                ...
                <button
                  ...
                  className="h-11 px-5 rounded-xl bg-slate-100 border border-slate-200 hover:bg-white/10 text-slate-700 font-bold uppercase text-[10px] tracking-wider transition-all cursor-pointer"
                >
                  Cập nhật phòng
                </button>
              </div>
              ...
              <form onSubmit={handleSaveRoom} className="space-y-5 text-xs text-left max-h-[60vh] overflow-y-auto pr-1 font-body font-semibold">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-slate-650 block text-[9px] uppercase tracking-wider">Tên căn hộ/phòng trọ</label>
                    <input
                      type="text"
                      value={formName}
                  ...
                  <div className="space-y-2">
                    <label className="text-slate-650 block text-[9px] uppercase tracking-wider">Trạng thái phòng trống</label>
                    <select
                      value={formStatus}
                ...
                <div className="space-y-2">
                  <label className="text-slate-650 block text-[9px] uppercase tracking-wider">Mô tả phòng chi tiết</label>
                  <textarea
                    value={formDesc}
                    onChange={(e) => setFormDesc(e.target.value)}
                    placeholder="Mô tả các chi tiết thêm về giờ giấc, quy định chung..."
                    rows={3}
                    className="w-full border border-slate-200 bg-background rounded-xl p-4 text-slate-700 outline-none focus:border-primary/50 resize-none"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    <label className="text-slate-650 block text-[9px] uppercase tracking-wider">Giá thuê (VND/tháng)</label>
                    <input
                      type="number"
                      value={formPrice}
                  ...
                  <div className="space-y-2">
                    <label className="text-slate-650 block text-[9px] uppercase tracking-wider">Tiền đặt cọc (VND)</label>
                    <input
                      type="number"
                      value={formDeposit}
                  ...
                  <div className="space-y-2">
                    <label className="text-slate-650 block text-[9px] uppercase tracking-wider">Diện tích (m²)</label>
                    <input
                      type="number"
                      value={formArea}
                ...
                <div className="space-y-2 pt-2 border-t border-slate-200">
                  <label className="text-slate-650 block text-[9px] uppercase tracking-wider font-extrabold text-emerald-400">Thiết lập địa chỉ phòng trọ</label>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <label className="text-slate-650 block text-[8px] uppercase tracking-wider">Số nhà, tên đường</label>
                      <input
                        type="text"
                        value={formStreet}
                    ...
                    <div className="space-y-2">
                      <label className="text-slate-650 block text-[8px] uppercase tracking-wider">Phường/Xã</label>
                      <input
                        type="text"
                        value={formWard}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <label className="text-slate-650 block text-[8px] uppercase tracking-wider">Quận/Huyện</label>
                      <input
                        type="text"
                        value={formDistrict}
                    ...
                    <div className="space-y-2">
                      <label className="text-slate-650 block text-[8px] uppercase tracking-wider">Tỉnh/Thành phố</label>
                      <input
                        type="text"
                        value={formCity}
                ...
                <div className="space-y-2 pt-2 border-t border-slate-200">
                  <label className="text-slate-650 block text-[9px] uppercase tracking-wider">Chọn đặc quyền & tiện ích</label>
                      ...
                      return (
                        <button
                          ...
                          className={`h-9 rounded-lg text-[9px] font-bold transition-all border text-center cursor-pointer active:scale-95 flex items-center justify-center ${
                            isSelected
                              ? "bg-primary/10 border-primary/30 text-primary"
                              : "bg-background border-slate-200 text-slate-650 hover:text-slate-350"
                          }`}
                        >
                          {item}
                        </button>
                ...
                <div className="pt-4 border-t border-slate-200 flex justify-end gap-3">
                  <button
                    type="button"
                    onClick={() => setIsFormOpen(false)}
                    className="h-11 px-5 rounded-xl bg-slate-100 border border-slate-200 hover:bg-white/10 text-slate-700 font-bold uppercase text-[9px] tracking-wider transition-all cursor-pointer"
                  >
                    Hủy bỏ
                  </button>
                  <button
                    type="submit"
                    disabled={saving}
                    className="h-11 px-6 rounded-xl bg-primary hover:bg-primary/90 text-white font-bold uppercase text-[9px] tracking-wider transition-all cursor-pointer flex items-center gap-1.5 shadow-md shadow-primary/10"
                  >
                    {saving ? (
                      <>
